---
openapi: "crm-v2 POST /api/unifications/crm/accounts/batch-read"
---
...
```bash Request
curl --request POST \
  --url https://api.vessel.dev/unifications/crm/accounts/batch-read \
...
for (let i = 0; i < ids.length; i += 200) {
  ...
  const batchResults = await fetch('https://api.vessel.dev/crm/accounts/batch-read',
  {
    ...,
    body: {
      ids: batchIds,
