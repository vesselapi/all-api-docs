...
```bash Terminal
curl -X POST https://api.vessel.dev/link/token
...
app.post("/link-token", async (req, res) => {
  ...
  const response = await fetch("https://api.vessel.dev/link/token", {
    method: "POST",
    headers: {
...
app.post("/store-token", async (req, res) => {
  ...
  const response = await fetch("https://api.vessel.dev/link/exchange", {
    method: "POST",
    headers: {
      "vessel-api-token": process.env.VESSEL_API_TOKEN,
    },
...
app.get("contacts", async (req, res) => {
  ...
  const response = await fetch(
    `https://api.vessel.dev/crm/contacts?accessToken=${accessToken}`,
    {
      method: "GET",
      headers: {
        "vessel-api-token": process.env.VESSEL_API_TOKEN,
