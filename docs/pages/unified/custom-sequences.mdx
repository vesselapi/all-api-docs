...
With the Vessel API, you can build this use case in a few simple steps, and also automatically enroll your customer's contacts into the sequence.
...
These custom fields are what allow us to 'embed' your custom text into the emails that are sent out.

You'll need to create a custom field for every piece of customized text that you want to add to the emails.

Also, please note that you'll need to make an exception for Outreach which doesn't allow for custom fields to be created. In the case that your customer connected Outreach, you'll need to use one of the predefined custom fields that Outreach provides. It's best practice to ask your user which custom field you can use to avoid accidently overriding a custom field used by a different integration. Your customer will thank you. This is the only special handling you'll need to do for this use case.
...
Note that there's a lot of different filtering options, don't try to enrich every contact in your customer's Sales Engagement Platform. In the example below, we're only getting the contacts that have a specific tag.
...
Next, fill in the custom field with the personalization data. You'll need to do this for each contact that you want to enrich.
