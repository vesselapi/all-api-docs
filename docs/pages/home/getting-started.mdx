...
<Warning>
  ...
  this guide [here](pages/unified/crm/getting-started)
</Warning>
...
The pseudo-code in this tutorial utilizes express and React.

### Pre-requisites

Before diving into this guide, make sure to complete the following:

1. [Create a free Vessel account](https://vessel.dev) and retrieve your development API key. Make sure to do the quick connect tutorial as well!
2. Ensure you have access to an account or test account for an integration we support (e.g. Outreach, Mailchimp, Aircall, etc.).
3. If you don't have access to an account, we recommend signing up for a free [Salesforce Developer](https://developer.salesforce.com/signup) account which will include access to the marketing APIs.

# Setup

### Step 1: Setting up the Auth UI

![Outreach connect](/images/outreach.png)

To start, we'll use Vessel's client sdk to quickly setup a front end component for authenticating your user. The client SDK adds a pre-built UI popup that walks your user through connecting to one of the many integrations we support.
...
<Note>
  At Vessel, security is our top priority. Besides following industry data
  security best practices, we've also crafted an authentication flow optimized
  for security and ease of use. If you have any questions or concerns, please
  reach out to our dedicated security email at `security@vessel.dev` and we'll
  reply in 24 hours.
</Note>
...
You're now ready to make an API call, feel free to browse on of the many unified or action APIs we support and pick one for the integration of the connection you just created.

```bash Terminal
curl POST 'https://app.vessel.dev/api/actions/apollo/users/search' \
