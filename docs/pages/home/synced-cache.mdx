...
This works great when you don’t need to read a lot of data or make many API calls. But what happens if you want to read your customer’s entire Salesforce instance or you need to make many API read calls in rapid succession? You run the risk of exhausting your customer’s rate limits, which can cause your integration to fail.
...
In summary, if you want to forgo issues with rate-limits, or want access to powerful features like filtering, synced-caches are an excellent tool to use. However, if your product is sensistive to data being out of sync by up to an hour, you may want to consider using the direct API calls instead.
...
You can set up a [webhook](/pages/home/webhooks) to be notified when the data is done syncing.

### Poll the Connections endpoint

Here’s what you can expect in the connections response to see the status of the sync:
...
# Synced-Cache Support

To view which integrations support synced-caches, check out the unified API pages and look for the "Synced-Cache" column in the "Integrations".

At this time, the Synced-Cache is only support for our Unified APIs.
