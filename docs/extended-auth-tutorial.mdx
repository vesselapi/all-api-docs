...
to create a new connection. The authentication process we employ is unique
to Unified API's, so don't fret if you're not familiar with it.
...
once the user has authenticated their CRM. Your backend will then exchange this
for an `accessToken` and a `connectionId`.

3. `linkToken`: This is the precursor to the `publicToken`. This is created
when the user initiates the connection flow, even before they
have authenticated their CRM. This will be used by Vessel's modal
to link the CRM authentication with our generated public token.
...
with their CRM's authentication flow, which then talks directly
