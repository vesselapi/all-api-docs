...
- Any time any object is updated/created/deleted, we'll send an event. You can use the `objectType` on the event payload to ignore objects that aren't relevant to your use case.
...
```json
{
  ...
  "type": "OBJECT_CHANGE",
  "data": {
    "objectType": string, // "Contact" | "Account" | "Deal" | "Lead" | "User" | "Note" | "Task" | "Event" | "EventAttendee"
...
app.post(WEBHOOK_URL, async (req, res) => {
  ...
  if (sha256(signature) !== req.headers["x-vessel-signature"]) {
