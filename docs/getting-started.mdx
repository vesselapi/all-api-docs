...
It is also recommended that you save the `connectionId`. The `connectionId` is similar to a public key and is used when referencing the state of the connection via the `/connection/` endpoints (e.g., to tell if the connection is syncing or if it's fully synced).
